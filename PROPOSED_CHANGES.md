...
- ~~**CI-Verankerung des Lint-Laufs** steht noch aus.~~ ✅ erledigt (2026-06-22): GitHub-Actions-Workflow `.github/workflows/ci.yml` (Job `quality-gate`) führt bei jedem Pull Request sowie bei Push auf `master`/`develop` nacheinander `npm run lint` → `npm test` → `npm run build` aus (Node 24 via `.nvmrc`, `npm ci --force` wegen der Peer-Dependency-Konflikte, npm-Cache). Lokal unter Node 24 verifiziert grün: lint 0 errors, test 83 passed/1 skipped (124 Tests), build EXIT 0. **Bewusst (noch) nicht im Gate:** `format:check` — fällt aktuell auf 443 unformatierten Bestands-Dateien, separater Strang. Optional als Folgeschritt: `quality-gate` als _Required status check_ in den Branch-Protection-Rules von `master`/`develop` setzen (GitHub-UI).
