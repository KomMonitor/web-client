...
**B4 · Keyword-/Typ-Filter (Cluster I).** 🚧 **teilweise.** Der **Georesource-Teil** (`onChangeGeoresourceKeywordFilter`, `getGeoresourceDatasets`, `getAvailable*`, `filterGeoresourcesByTypes`) wurde wegen der gegenseitigen Kopplung mit `setGeoresources` **zusammen mit B6e** in den `GeoresourceMetadataStoreService` gezogen (siehe B6e ✅). **Offen:** nur noch der **Indicator-Keyword-Filter** (`onChangeIndicatorKeywordFilter`, `filterIndicators`, `filterArrayObjectsByValue`, `displayableIndicators_keywordFiltered`) — klein; injiziert Indicator-/Topic-/ProcessScript-/HierarchyStore.
...
> **Offen (B6/Teil B):** nur noch der **Indicator-Keyword-Filter** (B4-Rest: `onChangeIndicatorKeywordFilter`, `filterIndicators`, `filterArrayObjectsByValue`, `displayableIndicators_keywordFiltered`) + **B7 Selection**.
