...
> **Verifizierter, umsetzbarer Plan + Fortschritt jetzt in eigener Doc:** [`ADMIN_AREA_BRIDGE_MIGRATION.md`](ADMIN_AREA_BRIDGE_MIGRATION.md). **Modals 4 + 1 + 2 ✅ erledigt (2026-06-22)** (`indicator-edit-indicator-spatial-unit-roles-modal`; `georesource-add-modal` inkl. Porting-Lücke `getCurrentKomMonitorLoginRoleIds` → `[]`; `georesource-edit-features-modal` über `FeatureTableDataGridHelperService`). Korrektur zur Tabelle unten: Modal 4 öffnet **nicht** via `NgbModal.open`, sondern via Broadcast + jQuery `$('#…').modal('show')` (unverändert gelassen). Offen: **nur noch Modal 3** (`indicator-edit-features-modal`, Porting-Lücken `buildDataGrid_featureTable_indicatorResource` + `$http`→`HttpClient`).
