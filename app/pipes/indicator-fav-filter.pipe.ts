...
export class IndicatorFavFilter implements PipeTransform {
  transform(
    ...
    topics = topics.filter(
      (elem) => elem.indicatorCount > 0 || elem.wmsCount > 0,
    );
  ...
  private topicOrIndicatorInFavRecursive(
    ...
    return tree.some(
      (elem) =>
        ...
        elem.indicatorData.some((i) =>
          indicatorFavItems.includes(i.indicatorId),
        ) ||
  ...
  private topicInFavTopBottom(
    ...
    while (parentNext !== undefined) {
      if (this.parentInFavRecursive(favTopicsTree, parentNext, topicFavItems))
        return true;
  ...
  private parentInFavRecursive(
    ...
    return tree.some((elem) => {
      if (elem.topicId === parentId && topicFavItems.includes(parentId))
        return true;
      if (elem.subTopics.length > 0)
        return this.parentInFavRecursive(
          elem.subTopics,
          parentId,
          topicFavItems,
        );
