 ...
 */
angular.module('kommonitorMap').service('kommonitorMapService', ['$rootScope', '$http', function ($rootScope, $http) {
        // // central map object
        // this.map;
        //
        ...
        this.addSpatialUnitGeoJSON = function (spatialUnitMetadataAndGeoJSON, date) {
            $rootScope.$broadcast("addSpatialUnitAsGeoJSON", spatialUnitMetadataAndGeoJSON, date);
        };
        this.addSpatialUnitWFS = function (name, wfsUrl) {
            $rootScope.$broadcast("addSpatialUnitAsWFS", name, wfsUrl);
        };
        this.addGeoresourceGeoJSON = function (georesourceMetadataAndGeoJSON, date) {
            $rootScope.$broadcast("addGeoresourceAsGeoJSON", georesourceMetadataAndGeoJSON, date);
        };
        this.addPoiGeoresourceGeoJSON = function (georesourceMetadataAndGeoJSON, date, useCluster) {
            $rootScope.$broadcast("addPoiGeoresourceAsGeoJSON", georesourceMetadataAndGeoJSON, date, useCluster);
        };
        this.addPoiGeoresourceGeoJSON_reachabilityAnalysis = function (georesourceMetadataAndGeoJSON, date, useCluster) {
            $rootScope.$broadcast("addPoiGeoresourceAsGeoJSON_reachabilityAnalysis", georesourceMetadataAndGeoJSON, date, useCluster);
        };
        this.addLoiGeoresourceGeoJSON = function (georesourceMetadataAndGeoJSON, date) {
            $rootScope.$broadcast("addLoiGeoresourceAsGeoJSON", georesourceMetadataAndGeoJSON, date);
        };
        this.addAoiGeoresourceGeoJSON = function (georesourceMetadataAndGeoJSON, date) {
            $rootScope.$broadcast("addAoiGeoresourceAsGeoJSON", georesourceMetadataAndGeoJSON, date);
        };
        this.replaceIsochroneGeoJSON = function (geoJSON, transitMode, reachMode, cutOffValues, useMultipleStartPoints, dissolveIsochrones, speedInKilometersPerHour) {
            $rootScope.$broadcast("replaceIsochronesAsGeoJSON", geoJSON, transitMode, reachMode, cutOffValues, useMultipleStartPoints, dissolveIsochrones, speedInKilometersPerHour);
        };
        this.replaceRouteGeoJSON = function (geoJSON, transitMode, preference, routingStartPoint, routingEndPoint, routeDistance_km, routeDuration_minutes) {
            $rootScope.$broadcast("replaceRouteAsGeoJSON", geoJSON, transitMode, preference, routingStartPoint, routingEndPoint, routeDistance_km, routeDuration_minutes);
        };
        this.replaceIsochroneMarker = function (lonLatArray) {
            $rootScope.$broadcast("replaceIsochroneMarker", lonLatArray);
        };
        this.removePoiGeoresource = function (georesourceMetadataAndGeoJSON) {
            $rootScope.$broadcast("removePoiGeoresource", georesourceMetadataAndGeoJSON);
        };
        this.removePoiGeoresource_reachabilityAnalysis = function (georesourceMetadataAndGeoJSON) {
            $rootScope.$broadcast("removePoiGeoresource_reachabilityAnalysis", georesourceMetadataAndGeoJSON);
        };
        this.removeLoiGeoresource = function (georesourceMetadataAndGeoJSON) {
            $rootScope.$broadcast("removeLoiGeoresource", georesourceMetadataAndGeoJSON);
        };
        this.removeAoiGeoresource = function (georesourceMetadataAndGeoJSON) {
            $rootScope.$broadcast("removeAoiGeoresource", georesourceMetadataAndGeoJSON);
        };
        ...
        this.addIndicatorGeoJSON = function (indicatorMetadataAndGeoJSON, spatialUnitName, date) {
            $rootScope.$broadcast("addIndicatorAsGeoJSON", indicatorMetadataAndGeoJSON, spatialUnitName, date);
        };
        this.replaceIndicatorGeoJSON = function (indicatorMetadataAndGeoJSON, spatialUnitName, date, justRestyling, isCustomComputation) {
            $rootScope.$broadcast("replaceIndicatorAsGeoJSON", indicatorMetadataAndGeoJSON, spatialUnitName, date, justRestyling, isCustomComputation);
        };
        this.addCustomIndicatorGeoJSON = function (indicatorMetadataAndGeoJSON, spatialUnitName, date) {
            $rootScope.$broadcast("addCustomIndicatorAsGeoJSON", indicatorMetadataAndGeoJSON, spatialUnitName, date);
        };
        this.restyleCurrentLayer = function () {
            $rootScope.$broadcast("restyleCurrentLayer");
        };
        this.addWmsLayerToMap = function (dataset, opacity) {
            $rootScope.$broadcast("addWmsLayerToMap", dataset);
        };
        this.adjustOpacityForWmsLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustOpacityForWmsLayer", dataset, opacity);
        };
        this.adjustOpacityForAoiLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustOpacityForAoiLayer", dataset, opacity);
        };
        this.adjustOpacityForPoiLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustOpacityForPoiLayer", dataset, opacity);
        };
        this.adjustOpacityForLoiLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustOpacityForLoiLayer", dataset, opacity);
        };
        this.removeWmsLayerFromMap = function (dataset) {
            $rootScope.$broadcast("removeWmsLayerFromMap", dataset);
        };
        this.addWfsLayerToMap = function (dataset, opacity, useCluster) {
            $rootScope.$broadcast("addWfsLayerToMap", dataset, opacity, useCluster);
        };
        this.adjustOpacityForWfsLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustOpacityForWfsLayer", dataset, opacity);
        };
        this.adjustColorForWfsLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustColorForWfsLayer", dataset, opacity);
        };
        this.removeWfsLayerFromMap = function (dataset) {
            $rootScope.$broadcast("removeWfsLayerFromMap", dataset);
        };
        this.addFileLayerToMap = function (dataset, opacity) {
            $rootScope.$broadcast("addFileLayerToMap", dataset);
        };
        this.adjustOpacityForFileLayer = function (dataset, opacity) {
            $rootScope.$broadcast("adjustOpacityForFileLayer", dataset, opacity);
        };
        this.adjustColorForFileLayer = function (dataset, color) {
            $rootScope.$broadcast("adjustColorForFileLayer", dataset, color);
        };
        this.removeFileLayerFromMap = function (dataset) {
            $rootScope.$broadcast("removeFileLayerFromMap", dataset);
        };
        this.addBaseDataLayer = function (selectedBaseData) {
            $rootScope.$broadcast("addBaseDataLayerAsWMS", selectedBaseData);
        };
        this.addIndicatorLayer = function (selectedIndicator) {
            $rootScope.$broadcast("addIndicatorLayerAsWMS", selectedIndicator);
        };
        ...
        this.processAsWMS = function (complexOutput, currentNameForLayerProperty) {
             ...
             * calls the associated event/method from wps-map controller!
             */
            $rootScope.$broadcast("addWMSOutput", { wmsURL: wmsBaseURL,
        ...
        function getUrlQueryParameterValueByName(parameterName, url) {
            ...
            var regex = new RegExp("[?&]" + parameterName + "(=([^&#]*)|&|#|$)"), resultsArray = regex.exec(url);
            if (!resultsArray)
                return null;
            if (!resultsArray[2])
                return '';
...
//# sourceMappingURL=kommonitor-map.module.js.map