...
angular.module('kommonitorMap').component('kommonitorMap', {
    ...
    controller: [
        ...
        function MapController($rootScope, $http, $scope, $timeout, kommonitorMapService, kommonitorDataExchangeService, kommonitorVisualStyleHelperService, kommonitorInfoLegendHelperService, kommonitorFilterHelperService, __env) {
                    ...
                    if (featureLayers.length === 0) {
                        ...
                        $scope.searchControl = new MultipleResultsLeafletSearch({
                            ...
                            formatData: function (json) {
                                var propName = this.options.propertyName, propLoc = this.options.propertyLoc, i, jsonret = {};
                                if (L.Util.isArray(propLoc))
                                    for (i in json) {
                                        if (!this._getPath(json[i], propName))
                                            continue;
            ...
            $scope.$on("replaceRouteAsGeoJSON", function (event, geoJSON, transitMode, preference, routingStartPoint, routingEndPoint, routeDistance_km, routeDuration_minutes) {
            ...
            $scope.$on("addFileLayerToMap", function (event, dataset, opacity) {
                ...
                if (fileType.toUpperCase() === "geojson".toUpperCase()) {
                    ...
                    shp(dataset.content).then(function (geojson) {
...
//# sourceMappingURL=kommonitor-map.component.js.map