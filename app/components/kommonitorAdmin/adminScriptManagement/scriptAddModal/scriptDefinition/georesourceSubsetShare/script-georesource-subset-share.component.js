			...
			*/
			...
			$scope.resetComputationFormulaAndLegend = function(){
				...
				if ($scope.propertyName !== undefined && $scope.operator && $scope.operator.apiName !== undefined && ($scope.propertyValue != undefined || $scope.propertyValueSelection != undefined)) {
					formulaHTML = "<b>Berechnung gem&auml;&szlig; Geodatenanalyse<br/><i>Prozentualer Anteil der Auswahl an allen Punkten des Datensatzes G<sub>1</sub> pro Raumebenen-Raumeinheit</i> <br/> <i>Auswahlkriterium:</i> '" + $scope.propertyName + "' '" + $scope.operator.displayName + "' '" + $scope.propertyValue + "'";
					if ($scope.operator.apiName === "Range") {
						formulaHTML = "<b>Berechnung gem&auml;&szlig; Geodatenanalyse<br/><i>Prozentualer Anteil der Auswahl an allen Punkten des Datensatzes G<sub>1</sub> pro Raumebenen-Raumeinheit</i> <br/> <i>Auswahlkriterium:</i> '" + $scope.propertyName + "' im Wertebereich von '>=" +  $scope.propertyValueRange_from + " bis <" + $scope.propertyValueRange_to + "'";
					}
					if ($scope.operator.apiName === "Contains") {
						formulaHTML = "<b>Berechnung gem&auml;&szlig; Geodatenanalyse<br/><i>Prozentualer Anteil der Auswahl an allen Punkten des Datensatzes G<sub>1</sub> pro Raumebenen-Raumeinheit</i> <br/> <i>Filterkriterium:</i> '" + $scope.propertyName + "' 'enthält' '" + $scope.propertyValueSelection + "'";
					}
				}
				else {
					formulaHTML = "<b>Berechnung gem&auml;&szlig; Geodatenanalyse<br/><i>Prozentualer Anteil der Auswahl an allen Punkten des Datensatzes G<sub>1</sub> pro Raumebenen-Raumeinheit</i>";
				}
