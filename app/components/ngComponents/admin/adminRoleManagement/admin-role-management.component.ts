...
export class AdminRoleManagementComponent implements OnInit {
  ...
  public columnDefs: ColDef<AccessControlTableEntry>[] = [
    {
      ...
      cellRendererParams: {
        onEditMetadata: (dataset: AccessControlTableEntry) =>
          this.openEditMetadataModal(dataset),
        onDelete: (dataset: AccessControlTableEntry) =>
  ...
  constructor(
    ...
    protected keycloakHelperService: KeycloakHelperService,
  ) {}
  ...
  private fetchAccessControlData(useCache): void {
    ...
    this.kommonitorDataExchangeService
      .fetchAccessControlMetadata(useCache)
      .subscribe({
  ...
  private applyTableFilter(): void {
    if (this.tableViewSwitcher) {
      this.rowData = this.allAccessControl.filter(
        (entry) => entry.datasetOwner === true,
      );
    } else {
  ...
  openEditMetadataModal(dataset: AccessControlMetadata): void {
    ...
    modalRef.componentInstance.currentDataset = JSON.parse(
  ...
  selectionChanged(
    $event: SelectionChangedEvent<AccessControlTableEntry, any>,
  ) {
