...
export class RoleAddModalComponent implements OnInit {
  ...
  private buildRoleDelegatesTable(): void {
    ...
    this.roleDelegatesColumnDefs = this.buildAdvancedRoleManagementColumnConfig();
  ...
  private getSelectedDelegatedPermissionIds(): string[] {
    ...
    if (this.roleDelegatesGridApi && !this.roleDelegatesGridApi.isDestroyed?.()) {
      this.roleDelegatesGridApi.forEachNode((node: any) => collectPermissions(node.data));
    } else {
  ...
  private buildRoleDelegatesPutBody(): Array<{
    ...
    this.getSelectedDelegatedPermissionIds().forEach((permissionId) => {
      const matchingLevel = ADVANCED_PERMISSION_GROUPS.flatMap((group) => [
        group.groupLevel,
        group.subGroupLevel,
      ]).find((permissionLevel) => permissionId.endsWith(`-${permissionLevel}`));

      if (!matchingLevel) {
        return;
      }

      const organizationalUnitId = permissionId.slice(0, -(matchingLevel.length + 1));

      const currentRoles = rolesByOrganizationalUnit.get(organizationalUnitId) || [];
    ...
    return Array.from(rolesByOrganizationalUnit.entries()).map(
      ([organizationalUnitId, adminRoles]) => {
        const organizationalUnit =
          this.kommonitorDataExchangeService.getAccessControlById(organizationalUnitId);

        return {
          organizationalUnitId,
          organizationalUnitName: organizationalUnit?.name || organizationalUnitId,
