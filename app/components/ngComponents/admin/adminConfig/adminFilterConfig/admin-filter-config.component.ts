...
export class AdminFilterConfigComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.httpClient.get('./config/filter-config_backup_forAdminViewExplanation.txt', {'responseType': 'text'}).subscribe({
      next: response => {
        this.filterConfigTemplate = response;

        this.kommonitorScriptHelperService.prettifyScriptCodePreview("filterConfig_backupTemplate"); 
      }
    });

      // set in app.js
    this.filterConfigTmp = JSON.stringify(window.__env.filterConfig, null, "    ");
    this.filterConfigCurrent = JSON.stringify(window.__env.filterConfig, null, "    ");
    this.filterConfigNew = JSON.stringify(window.__env.filterConfig, null, "    ");
  ...
  async onGlobalFilterDelete([itemId]) {
    ...
    if(item.length==1) {
      ...
      if(confirm("Wollen Sie den Filter "+item[0].name+" sicher dauerhaft löschen?")) {
        ...
        let configNew = this.origConfig.filter((e,i) => i!=itemId).map(e => { 
          delete e.filterId; 
          return e;
        });
        var addConfigResponse = await this.kommonitorConfigStorageService.postFilterConfig(JSON.stringify(configNew, null, "    "));	
  ...
  async editFilterConfig(){
    ...
    try {
      var addConfigResponse = await this.kommonitorConfigStorageService.postFilterConfig(this.filterConfigTmp).subscribe({
        next: async response => {
