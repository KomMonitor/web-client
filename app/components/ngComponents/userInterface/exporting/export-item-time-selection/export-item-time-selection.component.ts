...
export class ExportItemTimeSelectionComponent {
  ...
  get filteredStartTimestamps(): string[] {
    ...
    return end ? this.sortedTimestamps.filter((ts) => ts <= end) : this.sortedTimestamps;
  }

  get filteredEndTimestamps(): string[] {
    const start = this.rangeStart;
    return start ? this.sortedTimestamps.filter((ts) => ts >= start) : this.sortedTimestamps;
  }

  updateSingleTimestamp($event: Event) {
    const value = ($event.target as HTMLSelectElement).value;
    this.exportItem.selectedTargetTime = value ? { mode: 'point', value } : undefined;
  }
  ...
  setLatestEnd() {
    ...
    const current = this.exportItem.selectedTargetTime;
    const start = current?.mode === 'range' ? current.start : '';
    this.exportItem.selectedTargetTime = {
      mode: 'range',
      start,
