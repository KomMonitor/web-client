...
export class ExportingStateService {
  ...
  isSingleExportValid = computed(() => {
    const hasValidIndicator = this.indicatorItems().some(
      (item) => item.selectedSpatialUnitIds.length > 0 && item.selectedFormats.length > 0
    );
  ...
  isMultipleExportValid = computed(() =>
    this.indicatorItems().some(
      (item) =>
        item.selectedFormats.includes('GeoPackage') && item.selectedSpatialUnitIds.length > 0
    )
  );
  ...
  commonSpatialUnits = computed<SpatialUnit[]>(() => {
    ...
    return firstUnits.filter((unit) =>
      items.every((item) => item.dataset.spatialUnits.some((u) => u.id === unit.id))
    );
  });

  constructor() {
    effect(() => {
      localStorage.setItem(LS_INDICATOR_ITEMS, JSON.stringify(this.indicatorItems()));
    });
    effect(() => {
      localStorage.setItem(LS_GEORESSOURCE_ITEMS, JSON.stringify(this.georessourceItems()));
    });
  }

  removeIndicator(indicatorId: string) {
    this.indicatorItems.update((items) => items.filter((item) => item.dataset.id !== indicatorId));
  }

  removeGeoressource(datasetId: string): void {
    this.georessourceItems.update((items) => items.filter((item) => item.dataset.id !== datasetId));
  }
  ...
  addIndicator(indicator: Indicator): void {
    const alreadyExists = this.indicatorItems().some((item) => item.dataset.id === indicator.id);
