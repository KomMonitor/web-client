...
export class IndicatorRadarComponent implements OnInit {
  ...
  ngOnInit(): void {
    setTimeout(() => {
      ...
      this.propertiesForCurrentlySelectedIndicator =
        this.diagramHelperService.indicatorPropertiesForCurrentSpatialUnitAndTime.filter(
          (e) =>
            e.indicatorMetadata.indicatorId === this.selectionState.selectedIndicator.indicatorId
        );
      this.propertiesForBaseIndicatorsOfCurrentHeadlineIndicator =
        this.diagramHelperService.indicatorPropertiesForCurrentSpatialUnitAndTime.filter((e) => {
          const headlineIndicatorEntry = this.topicHierarchyStore.headlineIndicatorHierarchy.filter(
            (element) =>
              element.headlineIndicator.indicatorId ==
              this.selectionState.selectedIndicator.indicatorId
          )[0];

          if (headlineIndicatorEntry) {
            const baseIndicators_filtered = headlineIndicatorEntry.baseIndicators.filter(
              (element) => element.indicatorId == e.indicatorMetadata.indicatorId
            );
...
*/
  ...
  updateRadarChart(indicatorMetadataAndGeoJSON, spatialUnitName, spatialUnitId, date) {
    ...
    if (!this.radarChart) this.radarChart = echarts.init(document.getElementById('radarDiagram'));
    else {
      ...
      for (const indicatorForRadar of indicatorsForRadar) {
        if (indicatorForRadar.isSelected) {
          ...
          if (
            ...
            indicatorProperties = indicatorProperties.filter(
              (featureProperties) =>
          ...
          tooltip: {
            ...
            formatter: (params) => {
              ...
              for (let index = 0; index < params.value.length; index++) {
                string +=
                  this.radarOption.radar.indicator[index].name +
                  ': ' +
                  this.getIndicatorValue_asFormattedText(
                    params.value[index],
                    this.radarOption.radar.indicator[index].precision
                  ) +
                  ' [' +
                  this.radarOption.radar.indicator[index].unit +
                  ']<br/>';
              }
              return string;
            },
    ...
    for (const propertiesInstance of sampleProperties) {
      if (
        this.filterHelperService.featureIsCurrentlySelected(
          propertiesInstance[this.envConfigService.FEATURE_ID_PROPERTY_NAME]
        )
      ) {
  ...
  onUpdateDiagramsForHoveredFeature([featureProperties]) {
    if (
      ...
      !this.radarOption ||
      !this.radarOption.legend ||
      !this.radarOption.series
    ) {
      return;
    }

    const legendIndex = this.radarOption.legend.data.indexOf(
  ...
  checkResizeRadarChart() {
    ...
    } else this.radarHeight = this.radarheight_defaultNum + 'vh';
  }

  appendSeriesToRadarChart(featureProperties) {
    // append feature name to legend
    this.radarOption.legend.data.push(
      featureProperties[this.envConfigService.FEATURE_NAME_PROPERTY_NAME]
    );
      ...
      if (indicatorProperty.isSelected) {
        ...
        for (const indicatorPropertyInstance of indicatorProperties) {
          if (
            ...
            if (
              !this.indicatorValueService.indicatorValueIsNoData(
                indicatorPropertyInstance[this.DATE_PREFIX + date]
              )
            ) {
  ...
  highlightFeatureInRadarChart(featureProperties) {
    ...
    const dataIndex = this.getSeriesDataIndexByFeatureName(
      featureProperties[this.envConfigService.FEATURE_NAME_PROPERTY_NAME]
    );
    if (dataIndex > -1) {
  ...
  getSeriesDataIndexByFeatureName(featureName) {
    for (let index = 0; index < this.radarOption.series[0].data.length; index++) {
      if (this.radarOption.series[0].data[index].name == featureName) return index;
    }
  ...
  selectAllIndicatorsForRadar() {
    for (const indicator of this.diagramHelperService
      .indicatorPropertiesForCurrentSpatialUnitAndTime) {
