...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
	...
	if(temporalType === "YEARS"){
		resultMap = KmHelper.trend_consecutive_n_years(computationIndicator, targetDate, numberOfTemporalItems);
	}
	else if(temporalType === "MONTHS"){
		resultMap = KmHelper.trend_consecutive_n_months(computationIndicator, targetDate, numberOfTemporalItems);
	}
	else if(temporalType === "DAYS"){
		resultMap = KmHelper.trend_consecutive_n_days(computationIndicator, targetDate, numberOfTemporalItems);
	}
	else{
