...
async function computeIndicator(
  ...
  // now we compute the new indicator for the trend within n consecutive years
  // internally tests are run, e.g. if n previous years are available or not

  let resultMap = new Map();

  if (temporalType === 'YEARS') {
    resultMap = KmHelper.trend_consecutive_n_years(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  } else if (temporalType === 'MONTHS') {
    resultMap = KmHelper.trend_consecutive_n_months(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  } else if (temporalType === 'DAYS') {
    resultMap = KmHelper.trend_consecutive_n_days(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  ...
  KmHelper.log('Computed trend indicator for a total amount of ' + numFeatures + ' features');
  targetSpatialUnit_geoJSON = KmHelper.setIndicatorValues_fromIdValueMap(
    targetSpatialUnit_geoJSON,
    targetDate,
    resultMap
  );
  KmHelper.log('Computation of the trend finished');
