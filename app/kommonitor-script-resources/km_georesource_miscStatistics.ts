...
const computationProperty_name = 'compProp';
const computationMethod_name = 'compMeth';
...
async function computeIndicator(
  ...
  const computationProperty = KmHelper.getProcessParameterByName_asString(
    computationProperty_name,
    processParameters
  );
  const computationMethod = KmHelper.getProcessParameterByName_asString(
    computationMethod_name,
    processParameters
  );
  ...
  for (
    ...
    if (
      ...
      switch (computationMethod) {
        case 'SUM':
          KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, KmHelper.sum(valueArray));
          break;
        case 'MIN':
          KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, KmHelper.min(valueArray));
          break;
        case 'MAX':
          KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, KmHelper.max(valueArray));
          break;
        case 'MEAN':
          KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, KmHelper.mean(valueArray));
          break;
        case 'MEDIAN':
          KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, KmHelper.median(valueArray));
          break;
        case 'STANDARD_DEVIATION':
          KmHelper.setIndicatorValue(
            spatialUnitFeat,
            targetDate,
            KmHelper.standardDeviation(valueArray)
          );
          break;
        default:
          KmHelper.log(
            'Indicator was not computed from computation resources because no valid computation method was chosen. Indicator value is set to null.'
          );
