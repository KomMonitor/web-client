...
async function computeIndicator(
  ...
  const referenceIndicatorId = KmHelper.getProcessParameterByName_asString(
    parameterName_referenceIndicatorId,
    processParameters
  );
  const referenceGeoJSON = KmHelper.getBaseIndicatorById(referenceIndicatorId, baseIndicatorsMap);

  const computationIndicatorIds = KmHelper.getProcessParameterByName_asString(
    parameterName_computationIndicatorId,
    processParameters
  );
  const computationIndicatorIds_array = computationIndicatorIds.split(',');
  const computationGeoJSON_array = [];
  ...
  KmHelper.log("Process reference indicator '" + referenceIndicatorId + "'");
   ...
   */
  referenceGeoJSON.features.forEach(function (feature) {
    ...
    let referenceValue = KmHelper.getIndicatorValue(feature, targetDate);
  ...
  computationGeoJSON_array.forEach(function (geoJSON) {
    geoJSON.features.forEach(function (feature) {
      ...
      const featureId = KmHelper.getSpatialUnitFeatureIdValue(feature);
      // get the time series value of the base indicator feature for the requested target date (with its required prefix!)
      let partValue = KmHelper.getIndicatorValue(feature, targetDate);
      ...
      if (!map.has(featureId)) {
        KmHelper.log(
          "Computation Indicator feature with id '" +
            featureId +
            "' was not computed from computation resources. Will set ref value to null."
        );
        const mapObject = {
          featureId: featureId,
          indicatorValue: undefined,
  ...
  targetSpatialUnit_geoJSON.features.forEach(function (spatialUnitFeature) {
    ...
    if (!map.has(spatialUnitFeatureId)) {
      KmHelper.log(
        "Target spatial unit feature with id '" +
          ...
          "' was not computed from computation resources. Will set indicator value to null."
      );
      const mapObject = {
    ...
    if (
      mapEntry.refValue === undefined ||
      mapEntry.refValue === null ||
      mapEntry.refValue === undefined ||
      ...
      indicatorValue = (mapEntry.intermediateValue / mapEntry.refValue) * 100;
    }
