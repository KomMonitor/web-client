...
const parameterName_computationIndicatorId_numerator = 'COMPUTATION_ID_numerator';
const parameterName_computationIndicatorId_denominator = 'COMPUTATION_ID_denominator';
...
async function computeIndicator(
  ...
  const computationIndicatorId_numerator = KmHelper.getProcessParameterByName_asString(
    parameterName_computationIndicatorId_numerator,
    processParameters
  );
  const numeratorGeoJSON = KmHelper.getBaseIndicatorById(
    computationIndicatorId_numerator,
    baseIndicatorsMap
  );

  const computationIndicatorId_denominator = KmHelper.getProcessParameterByName_asString(
    parameterName_computationIndicatorId_denominator,
    processParameters
  );
  const denominatorGeoJSON = KmHelper.getBaseIndicatorById(
    computationIndicatorId_denominator,
    baseIndicatorsMap
  );
  ...
  KmHelper.log("Process numerator indicator '" + computationIndicatorId_numerator + "'");
   ...
   */
  numeratorGeoJSON.features.forEach(function (feature) {
    // get the unique featureID of the spatial unit feature as String
    const featureId = KmHelper.getSpatialUnitFeatureIdValue(feature);
    // get the time series value of the base indicator feature for the requested target date (with its required prefix!)
    let numeratorValue = KmHelper.getIndicatorValue(feature, targetDate);

    if (numeratorValue === undefined || numeratorValue === null) {
      KmHelper.log(
        "WARNING: the feature with featureID '" +
          featureId +
          "' does not contain a time series value for targetDate '" +
          targetDate +
          "'"
      );
      KmHelper.log(
        "WARNING: the feature value will thus be set to '0' and computation will continue"
      );
      numeratorValue = 0;
    }
    ...
    const mapEntry = map.get(featureId);
    mapEntry.numeratorValue = numeratorValue;
    map.set(featureId, mapEntry);
  });

  KmHelper.log('Process denominator indicator');

  /**
   * iterate over each feature of the baseIndicator and use its indicator value to modify map object
   * NOTE use spatialUnitFeatureId as key to be able to identify entries by their unique feature id!
   */
  denominatorGeoJSON.features.forEach(function (feature) {
    // get the unique featureID of the spatial unit feature as String
    const featureId = KmHelper.getSpatialUnitFeatureIdValue(feature);
    // get the time series value of the base indicator feature for the requested target date (with its required prefix!)
    let denominatorValue = KmHelper.getIndicatorValue(feature, targetDate);
    ...
    if (!map.has(featureId)) {
      KmHelper.log(
        "numeratorValue Indicator feature with id '" +
          featureId +
          "' was not computed from computation resources. Will set denominatorValue to null."
      );
      const mapObject = {
        featureId: featureId,
        indicatorValue: undefined,
  ...
  let spatialUnitIndex = 0;
  // create progress log after each 10th percent of features
  const logProgressIndexSeparator = Math.round((numFeatures / 100) * 10);
  targetSpatialUnit_geoJSON.features.forEach(function (spatialUnitFeature) {
    ...
    const spatialUnitFeatureId = KmHelper.getSpatialUnitFeatureIdValue(spatialUnitFeature);

    if (!map.has(spatialUnitFeatureId)) {
      KmHelper.log(
        "Target spatial unit feature with id '" +
          spatialUnitFeatureId +
          "' was not computed from computation resources. Will set nominator and denominator values to null."
      );
      const mapObject = {
    ...
    const mapEntry = map.get(spatialUnitFeatureId);
    let indicatorValue = null;

    if (
      mapEntry.numeratorValue === undefined ||
      mapEntry.numeratorValue === null ||
      mapEntry.denominatorValue === undefined ||
      mapEntry.denominatorValue === null
    ) {
      KmHelper.log(
        "WARNING: the feature with featureID '" +
          spatialUnitFeatureId +
      ...
      indicatorValue = null;
    } else if (mapEntry.denominatorValue === 0) {
      indicatorValue = 0;
      KmHelper.log(
        "WARNING: the feature with featureID '" +
          spatialUnitFeatureId +
          "' cotains a '0' as numerator value for targetDate '" +
          targetDate +
          "'"
      );
      KmHelper.log('INFO: indicator value is set to 0');
    } else {
      indicatorValue = mapEntry.numeratorValue / mapEntry.denominatorValue;
    }
    ...
    if (spatialUnitIndex % logProgressIndexSeparator === 0) {
      KmHelper.log(
        "PROGRESS: Computed '" + spatialUnitIndex + "' of total '" + numFeatures + "' features."
      );
    }
  });
