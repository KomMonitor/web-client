...
async function computeIndicator(
  ...
  for (
    ...
    const pointsWithinFeature = KmHelper.pointsWithinPolygon(
      computationGeoresource,
      spatialUnitFeat
    );
    if (
      pointsWithinFeature &&
      pointsWithinFeature.features &&
      pointsWithinFeature.features.length > 0
    ) {
      if (computationFilterProperty !== undefined) {
        const valueArray = KmHelper.getPropertyValueArray(
          pointsWithinFeature,
          computationFilterProperty
        );
        let filteredArray = [];
        switch (computationFilterOperator) {
          ...
          case 'Range': {
            const computationFilterPropertyValueArray = computationFilterPropertyValue
              .split('-')
              .map((el) => parseInt(el));
            filteredArray = valueArray.filter(
              (item) =>
                item >= computationFilterPropertyValueArray[0] &&
                item < computationFilterPropertyValueArray[1]
            );
            break;
          }
          default:
        ...
        const share = (filteredArray.length / valueArray.length) * 100;
        KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, share);
      } else {
        KmHelper.log(
          'Indicator was not computed from computation resources because no valid filter has been defined. Indicator value is set to null.'
        );
        KmHelper.setIndicatorValue(spatialUnitFeat, targetDate, null);
      }
    }
    ...
    if (featureIndex % logProgressIndexSeparator === 0) {
      KmHelper.log(
        "PROGRESS: Compared '" +
          ...
          "' spatial units to point features."
      );
    }
  }
