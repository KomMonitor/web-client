...
async function computeIndicator(
  ...
  const computationIndicatorId = KmHelper.getProcessParameterByName_asString(
    parameterName_computationIndicatorId,
    processParameters
  );
  const temporalType = KmHelper.getProcessParameterByName_asString(
    parameterName_temporalType,
    processParameters
  );
  const numberOfTemporalItems = KmHelper.getProcessParameterByName_asNumber(
    parameterName_numberOfTemporalItems,
    processParameters
  );

  // get the indicator using the computationIndicatorId
  const computationIndicator = KmHelper.getBaseIndicatorById(
    computationIndicatorId,
    baseIndicatorsMap
  );
  KmHelper.log('Retrieved required computationIndicator successfully.');

  // now we compute the new indicator for the continuity within n consecutive years
  // internally tests are run, e.g. if n previous years are available or not

  let resultMap = new Map();

  if (temporalType === 'YEARS') {
    resultMap = KmHelper.continuity_consecutive_n_years(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  } else if (temporalType === 'MONTHS') {
    resultMap = KmHelper.continuity_consecutive_n_months(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  } else if (temporalType === 'DAYS') {
    resultMap = KmHelper.continuity_consecutive_n_days(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
    ...
    KmHelper.log(
      "WARNING: parameter '" +
        parameterName_temporalType +
        "' with allowed values ['YEARS', 'MONTHS', 'DAYS'] has unknown value '" +
        temporalType +
        "'. Will use YEARS as default"
    );
    resultMap = KmHelper.continuity_consecutive_n_years(
      computationIndicator,
      targetDate,
  ...
  KmHelper.log('Computed continuity indicator for a total amount of ' + numFeatures + ' features');
  targetSpatialUnit_geoJSON = KmHelper.setIndicatorValues_fromIdValueMap(
    targetSpatialUnit_geoJSON,
    targetDate,
    resultMap
  );
  KmHelper.log('Computation of the continuity finished');
