...
async function computeIndicator(
  ...
  // now we compute the new indicator for a relative change compared to previous year
  // internally tests are run, e.g. if a previous year is available or not
  let resultMap = new Map();

  if (temporalType === 'YEARS') {
    resultMap = KmHelper.changeRelative_n_years_percent(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  } else if (temporalType === 'MONTHS') {
    resultMap = KmHelper.changeRelative_n_months_percent(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  } else if (temporalType === 'DAYS') {
    resultMap = KmHelper.changeRelative_n_days_percent(
      computationIndicator,
      targetDate,
      numberOfTemporalItems
    );
  ...
  KmHelper.log('Computation of relative change compared to previous year finished');
