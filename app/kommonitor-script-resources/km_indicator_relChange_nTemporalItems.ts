...
async function computeIndicator(targetDate, targetSpatialUnit_geoJSON, baseIndicatorsMap, georesourcesMap, processParameters){
	...
	if(temporalType === "YEARS"){
		resultMap = KmHelper.changeRelative_n_years_percent(computationIndicator, targetDate, numberOfTemporalItems);
	}
	else if(temporalType === "MONTHS"){
		resultMap = KmHelper.changeRelative_n_months_percent(computationIndicator, targetDate, numberOfTemporalItems);
	}
	else if(temporalType === "DAYS"){
		resultMap = KmHelper.changeRelative_n_days_percent(computationIndicator, targetDate, numberOfTemporalItems);
	}
	else{
