...
async function computeIndicator(
  ...
  const computationGeoresourceId = KmHelper.getProcessParameterByName_asString(
    computationGeoresourceId_name,
    processParameters
  );
  // retrieve required baseIndicator using its meaningful name
  let computationGeoresource = KmHelper.getGeoresourceById(
    computationGeoresourceId,
    georesourcesMap
  );
  ...
  let computationFilterProperty;
  let computationFilterOperator;
  let computationFilterPropertyValue;
  ...
  if (parameter === undefined) {
    ...
    computationFilterProperty = KmHelper.getProcessParameterByName_asString(
      computationFilterProperty_name,
      processParameters
    );
    computationFilterOperator = KmHelper.getProcessParameterByName_asString(
      computationFilterOperator_name,
      processParameters
    );
    computationFilterPropertyValue = KmHelper.getProcessParameterByName_asString(
      computationFilterPropertyValue_name,
      processParameters
    );
  }

  KmHelper.log(
    'calculating spatial intersection and within check between lines and target spatial unit.'
  );

  // create progress log after each 10th percent of features
  const logProgressIndexSeparator = Math.round(
    (targetSpatialUnit_geoJSON.features.length / 100) * 10
  );

  for (
    let featureIndex = 0;
    featureIndex < targetSpatialUnit_geoJSON.features.length;
    featureIndex++
  ) {
    ...
    const linesWithinFeature = KmHelper.intersectLineFeatureCollectionByPolygonFeature(
      computationGeoresource,
      spatialUnitFeat
    );
    if (
      linesWithinFeature &&
      linesWithinFeature.features &&
      linesWithinFeature.features.length > 0
    ) {
      ...
      if (computationFilterProperty !== undefined) {
        ...
        linesWithinFeature.features.forEach(function (line) {
          ...
          switch (computationFilterOperator) {
            ...
            case 'Contains':
              ...
              break;
            case 'Range':
              computationFilterPropertyValueArray = computationFilterPropertyValue
                .split('-')
                .map((el) => parseInt(el));
              if (
                line.properties[computationFilterProperty] >=
                  computationFilterPropertyValueArray[0] &&
                line.properties[computationFilterProperty] < computationFilterPropertyValueArray[1]
              ) {
                filteredLineFeatures.push(line);
              }
              break;
            default:
              KmHelper.log(
                'Indicator was not computed from computation resources because no valid filter could be applied. Indicator value is set to null.'
              );
        ...
        if (filteredLineFeatures.length === 0) {
          ...
          lineSegmentsLengthSum = KmHelper.summarizeLineSegmentLengths(
            KmHelper.asFeatureCollection(filteredLineFeatures)
          );
    ...
    if (featureIndex % logProgressIndexSeparator === 0) {
      KmHelper.log(
        "PROGRESS: Compared '" +
          ...
          "' spatial units to line features."
      );
    }
  }
