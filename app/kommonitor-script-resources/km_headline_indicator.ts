...
const parameterName_computationIndicatorIds = 'COMPUTATION_ID_ARRAY';
const parameterName_computationPolarity = 'COMPUTATION_POLARITY_ARRAY';
// supported methods: "geomean", "mean", "min"
const parameterName_aggregationMethod = 'AGGREGATION_METHOD';
...
async function computeIndicator(
  ...
  const computationIndicatorIds = KmHelper.getProcessParameterByName_asString(
    parameterName_computationIndicatorIds,
    processParameters
  );
  const computationPolarity = KmHelper.getProcessParameterByName_asString(
    parameterName_computationPolarity,
    processParameters
  );
  const aggregationMethod = KmHelper.getProcessParameterByName_asString(
    parameterName_aggregationMethod,
    processParameters
  );

  // get an array of the indicator ids
  const computationIndicatorIds_array = computationIndicatorIds.split(',');
  // get an array of the same length including the polarity of each indicator, 'regular' or 'inverted'
  const computationPolarity_array = computationPolarity.split(',');

  // get an array of indicators using the indicator ids
  const geoJSON_array = [];
  for (const indicatorId of computationIndicatorIds_array) {
    geoJSON_array.push(KmHelper.getBaseIndicatorById(indicatorId, baseIndicatorsMap));
  }
  KmHelper.log('Retrieved required baseIndicators successfully');

  // get an array of map objects with id-value pairs for each featureCollection in the array
  KmHelper.log(
    'Iterate over array of baseIndicators and create an array of map objects with id-value pairs for each featureCollection'
  );
  const indicator_idValueMap_array = [];
  geoJSON_array.forEach(function (geoJSON) {
    indicator_idValueMap_array.push(KmHelper.getIndicatorIdValueMap(geoJSON, targetDate));
  });
  KmHelper.log('Retrieved required map objects successfully');

  // get an array of ranked map objects with id-value pairs for each baseIndicator map in the array
  KmHelper.log(
    'Iterate over array of baseIndicator maps and create an array of ranked map objects'
  );
  const rankedIndicator_idValueMap_array = [];
  indicator_idValueMap_array.forEach(function (idValueMap) {
    rankedIndicator_idValueMap_array.push(KmHelper.rank_fromIdValueMap(idValueMap));
  });
  KmHelper.log('Retrieved ranked map objects successfully');

  // get an array of min-max normalized map objects with id-value pairs for each ranked indicator in the array considering the polarity of the indicator
  KmHelper.log(
    'Iterate over array ranked map objects and create an array of min-max normalized map objects'
  );
  let index;
  const minMaxNormalizedIndicator_idValueMap_array = [];
  for (index = 0; index < rankedIndicator_idValueMap_array.length; index++) {
    const polarity = computationPolarity_array[index];
    if (polarity === 'regular') {
      minMaxNormalizedIndicator_idValueMap_array.push(
        KmHelper.minMaxNormalization_fromIdValueMap(rankedIndicator_idValueMap_array[index])
      );
    } else {
      minMaxNormalizedIndicator_idValueMap_array.push(
        KmHelper.minMaxNormalization_inverted_fromIdValueMap(
          rankedIndicator_idValueMap_array[index]
        )
      );
    }
  }
  KmHelper.log('Retrieved min-max-normalized map objects successfully');

  // now we compute the new indicator by applying the method chosen by the user
  let resultMap;
  if (aggregationMethod === 'geomean') {
    // apply geometric mean to an array of min-max-normalized map objects
    KmHelper.log('Calculate geometric mean');
    resultMap = KmHelper.geomean_fromIdValueMap(minMaxNormalizedIndicator_idValueMap_array);
  }
  if (aggregationMethod === 'mean') {
    // apply mean to an array of min-max-normalized map objects
    KmHelper.log('Calculate mean');
    resultMap = KmHelper.mean_fromIdValueMap(minMaxNormalizedIndicator_idValueMap_array);
  }
  if (aggregationMethod === 'min') {
    // apply min to an array of min-max-normalized map objects
    KmHelper.log('Calculate min');
    resultMap = KmHelper.min_fromIdValueMap(minMaxNormalizedIndicator_idValueMap_array);
  }
  const numFeatures = targetSpatialUnit_geoJSON.features.length;
  KmHelper.log('Computed headline indicator for a total amount of ' + numFeatures + ' features');
  targetSpatialUnit_geoJSON = KmHelper.setIndicatorValues_fromIdValueMap(
    targetSpatialUnit_geoJSON,
    targetDate,
    resultMap
  );
  KmHelper.log('Computation of headline indicator finished');
...
function aggregateIndicator(targetDate, targetSpatialUnit_geoJSON, indicator_geoJSON) {
  ...
  if (!aggregationTypeEnum.includes(aggregationType)) {
    console.log(
      "Unknown parameter value for 'aggregationType' was specified for aggregation logic. Parameter value was '" +
        aggregationType +
        "'. Allowed values are: " +
        aggregationTypeEnum
    );
...
function aggregate_average(targetDate, targetSpatialUnit_geoJSON, indicator_geoJSON) {
  ...
  KmHelper.log(
    'Aggregate indicator for targetDate ' +
      targetDate +
      ' for a total amount of ' +
      targetSpatialUnit_geoJSON.features.length +
      ' target features. Computing AVERAGE values.'
  );
  KmHelper.log(
    'Aggregate from a total number of ' + indicator_geoJSON.features.length + ' baseFeatures'
  );
  KmHelper.log(
    'Aggregating by comparing the centroids of each indicator feature to target spatial unit features. Each indicator feature will be weighted by its size (area in squareMeters).'
  );
  ...
  targetSpatialUnit_geoJSON.features.forEach(function (targetFeature) {
    ...
    for (let index = 0; index < indicatorFeatures.length; index++) {
      ...
      if (KmHelper.within(indicatorFeature, targetFeature)) {
        ...
        if (
          !Number.isNaN(indicatorFeature.properties[targetDate]) &&
          indicatorFeature.properties[targetDate] !== null &&
          indicatorFeature.properties[targetDate] !== undefined
        ) {
          ...
          targetFeature.properties[targetDate] +=
            Number(indicatorFeature.properties[targetDate]) * weight;
        }
      }
    }
    ...
    if (baseIndicatorTotalWeight === 0) {
      ...
      targetFeature.properties[targetDate] =
        targetFeature.properties[targetDate] / baseIndicatorTotalWeight;
    }
  ...
  if (indicatorFeatures.length > 0) {
    ...
    throw Error(
      'Spatial Aggregation operation failed for a total number of ' + indicatorFeatures.length
    );
  }
...
function aggregate_sum(targetDate, targetSpatialUnit_geoJSON, indicator_geoJSON) {
  ...
  KmHelper.log(
    'Aggregate indicator for targetDate ' +
      ...
      ' for a total amount of ' +
      targetSpatialUnit_geoJSON.features.length +
      ' target features. Computing SUM values.'
  );
  KmHelper.log(
    'Aggregate from a total number of ' + indicator_geoJSON.features.length + ' baseFeatures'
  );
  KmHelper.log(
    'Aggregating by comparing the centroids of each indicator feature to target spatial unit features.'
  );
  ...
  KmHelper.log('Aggregation finished');
  KmHelper.log(
    totalAggregatedIndicatorFeatures +
      ' features were aggregated to ' +
      targetSpatialUnit_geoJSON.features.length +
      ' targetFeatures'
  );
