...
export class ReachabilityMapHelperService {
  ...
  constructor(
    ...
    private envConfService: EnvConfigService
  ) { }
  ...
  initReachabilityGeoMap(domId: string) {
    ...
    mapParts = this.genericMapHelperService.initMap(domId, true, true, false, true, undefined, undefined);
  ...
  zoomToIsochroneLayer(domId: string) {
    ...
    if (mapParts && mapParts.map && mapParts.isochroneLayers && mapParts.isochroneLayers.isochroneLayer) {
      this.genericMapHelperService.zoomToLayer(mapParts.map, mapParts.isochroneLayers.isochroneLayer);
    }
  }
  ...
  zoomToIndicatorLayer(domId: string) {
    ...
    if (mapParts && mapParts.map && mapParts.indicatorStatistics && mapParts.indicatorStatistics.indicatorLayer) {
      this.genericMapHelperService.zoomToLayer(mapParts.map, mapParts.indicatorStatistics.indicatorLayer);
    }
  }
  ...
  zoomToIndicatorFeature(domId: string, feature: any) {
    ...
    if (mapParts && mapParts.map && mapParts.indicatorStatistics && mapParts.indicatorStatistics.indicatorLayer) {
      for (const layerKey in mapParts.indicatorStatistics.indicatorLayer._layers) {
        if (Object.prototype.hasOwnProperty.call(mapParts.indicatorStatistics.indicatorLayer._layers, layerKey)) {
          const layer = mapParts.indicatorStatistics.indicatorLayer._layers[layerKey];
          if (layer.feature.properties[this.envConfigService.FEATURE_ID_PROPERTY_NAME] === feature.properties[this.envConfigService.FEATURE_ID_PROPERTY_NAME]) {
  ...
  makeIsochroneLayer(datasetName: string, geoJSON: any, transitMode: string, reachMode: string, cutOffValues: number[], useMultipleStartPoints: boolean, dissolveIsochrones: boolean): L.FeatureGroup {
    ...
    switch (transitMode) {
      case "buffer": transitModeValue = "Puffer (Luftlinie)"; break;
      case "cycling-regular": transitModeValue = "Fahrrad"; break;
      case "driving-car": transitModeValue = "PKW"; break;
      case "wheelchair": transitModeValue = "Barrierefrei"; break;
      default: transitModeValue = "Passant";
    }
  ...
  replaceIsochroneGeoJSON(domId: string, datasetName: string, geoJSON: any, transitMode: string, reachMode: string, cutOffValues: number[], useMultipleStartPoints: boolean, dissolveIsochrones: boolean) {
    const mapParts = this.mapPartsMap.get(domId);
  ...
  generatePoiMarkers(georesourceMetadataAndGeoJSON: any, useCluster: boolean, geojsonPropName: string): L.Layer {
    let markers: L.Layer;
    if (useCluster) {
      markers = (L as any).markerClusterGroup({
        iconCreateFunction: (cluster: any) => {
          ...
          return new L.DivIcon({ html: `<div class="awesome-marker-legend-icon-${georesourceMetadataAndGeoJSON.poiMarkerColor}"><span>${childCount}</span></div>`, className, iconSize: new L.Point(40, 40) });
        }
      });
    } else {
  ...
  addPoiGeoresourceGeoJSON_reachabilityAnalysis(domId: string, georesourceMetadataAndGeoJSON: any, date: string, useCluster: boolean) {
    const mapParts = this.mapPartsMap.get(domId);

    if (mapParts && mapParts.isochroneLayers && mapParts.isochroneLayers.poiInIsoLayers && mapParts.isochroneLayers.poiInIsoLayers.has(georesourceMetadataAndGeoJSON.georesourceId)) {
      const layer = mapParts.isochroneLayers.poiInIsoLayers.get(georesourceMetadataAndGeoJSON.georesourceId);
    ...
    const markers = this.generatePoiMarkers(georesourceMetadataAndGeoJSON, useCluster, "geoJSON_poiInIsochrones");
  ...
  async fetchIndicatorForSpatialUnit(indicatorId: string, spatialUnitId: string, timestamp: string): Promise<any> {
  ...
  onEachFeatureIndicator(feature: any, layer: L.Layer, indicatorProperty: string, indicatorStatisticsCandidate: any) {
    const indicatorValue = feature.properties[indicatorProperty];
    const indicatorValueText = this.dataExchangeService.indicatorValueIsNoData(indicatorValue) ? "NoData" : this.dataExchangeService.getIndicatorValue_asFormattedText(indicatorValue);
  ...
  async generateIndicatorLayer(indicatorMetadataAndGeoJSON: any, indicatorPropertyName: string, defaultBrew: any, indicatorStatisticsCandidate: any): Promise<L.GeoJSON> {
    ...
    const layer = L.geoJSON(indicatorMetadataAndGeoJSON.geoJSON, {
      style: (feature) => this.visualStyleHelperService.styleDefault(feature, defaultBrew, undefined, undefined, indicatorPropertyName, true, false, true),
      onEachFeature: (feature, layer) => this.onEachFeatureIndicator(feature, layer, indicatorPropertyName, indicatorStatisticsCandidate)
    });
  ...
  async setupIndicator(indicatorStatisticsCandidate: any) {
    ...
    const indicatorMetadataAndGeoJSON = this.dataExchangeService.getIndicatorMetadataById(indicatorId);
  ...
  async replaceReachabilityIndicatorStatisticsOnMap(domId: string, poiDataset: any, original_nonDissolved_isochrones: any, indicatorStatisticsCandidate: any) {
    const mapParts = this.mapPartsMap.get(domId);
    ...
    const defaultBrew = this.visualStyleHelperService.setupDefaultBrew(indicatorMetadataAndGeoJSON.geoJSON, indicatorPropertyName, indicatorMetadataAndGeoJSON.defaultClassificationMapping.numClasses, indicatorMetadataAndGeoJSON.defaultClassificationMapping.colorBrewerSchemeName, this.visualStyleHelperService.classifyMethod, true, indicatorMetadataAndGeoJSON);

    const indicatorLayer = await this.generateIndicatorLayer(indicatorMetadataAndGeoJSON, indicatorPropertyName, defaultBrew, indicatorStatisticsCandidate);
    ...
    const poiLayer = this.generatePoiLayerForIndicatorStatistic(poiDataset, original_nonDissolved_isochrones, indicatorStatisticsCandidate);
  ...
  generateIndicatorLegend(defaultBrew: any): L.Control {
    ...
    legend.onAdd = () => {
      const grades = defaultBrew.breaks.map((b: number) => this.dataExchangeService.getIndicatorValue_asFormattedText(b));
  ...
  getIndicatorFeature_forSpatialUnitFeatureId(indicatorGeoJSON: any, spatialUnitFeatureId: string): any {
    return indicatorGeoJSON.features.find((feature: any) => feature.properties[this.envConfigService.FEATURE_ID_PROPERTY_NAME] === spatialUnitFeatureId);
  }
  ...
  generatePoiPopupContent(poiFeature: any, indicatorStatisticsCandidate: any): string {
    ...
    for (const isochronePruneResult of poiFeature.properties.individualIsochronePruneResults) {
      ...
      const unit = this.reachabilityScenarioHelperService.tmpActiveScenario.reachabilitySettings.focus === 'distance' ? "Meter" : "Minuten";
      ...
      for (const spatialUnitCoverageEntry of isochronePruneResult.spatialUnitCoverage) {
        const indicatorFeature = this.getIndicatorFeature_forSpatialUnitFeatureId(indicatorStatisticsCandidate.indicator.geoJSON, spatialUnitCoverageEntry.spatialUnitFeatureId);
  ...
  onClickPoiMarker_indicatorStatistics(event: L.LeafletEvent) {
    ...
    this.addSinglePoiIsochroneLayer(this.domId_indicatorStatistics, feature, poiIsochroneLayer, false);
  }
  ...
  addSinglePoiIsochroneLayer(domId: string, feature: any, poiIsochroneLayer: L.FeatureGroup, zoomToLayer: boolean) {
    const mapParts = this.mapPartsMap.get(domId);
    mapParts.indicatorStatistics.poiIsochroneLayer = poiIsochroneLayer;
    mapParts.layerControl.addOverlay(mapParts.indicatorStatistics.poiIsochroneLayer, `Isochronen um Punkt '${feature.properties[this.envConfigService.FEATURE_NAME_PROPERTY_NAME]}'`);
  ...
  generateSinglePoiIsochroneLayer(feature: any): L.FeatureGroup {
    ...
    for (let i = isochrones.length - 1; i >= 0; i--) {
      const styleIndex = this.getStyleIndexForFeature(isochrones[i], this.dataExchangeService.isochroneLegend.colorValueEntries, this.dataExchangeService.isochroneLegend.reachMode_apiValue);
      const style = {
        color: this.dataExchangeService.isochroneLegend.colorValueEntries[styleIndex].color,
        weight: 1,
        opacity: 0.4,
  ...
  generatePoiMarkers_indicatorStatistics(poiDataset: any, indicatorStatisticsCandidate: any): L.Layer {
  ...
  generatePoiLayerForIndicatorStatistic(poiDataset: any, original_nonDissolved_isochrones: any, indicatorStatisticsCandidate: any): L.Layer {
  ...
  initPoiMap(poiGeoJSON: any): Map<string, any> {
    ...
    for (const poiFeature of poiGeoJSON.features) {
      ...
      poiMap.set(`${poiFeature.properties[this.envConfigService.FEATURE_ID_PROPERTY_NAME]}`, poiFeature);
    }
    return poiMap;
  }

  attachIndividualIsochronesToPOIs(poiMap: Map<string, any>, original_nonDissolved_isochrones: any): Map<string, any> {
    for (const isochroneFeature of original_nonDissolved_isochrones.features) {
      const poiFeatureID = isochroneFeature.properties[this.envConfigService.FEATURE_ID_PROPERTY_NAME].substring(0, isochroneFeature.properties[this.envConfigService.FEATURE_ID_PROPERTY_NAME].lastIndexOf("_"));
  ...
  attachIndividualIsochronePruneResultsToPOIs(poiMap: Map<string, any>, isochronePruneResults: any): Map<string, any> {
    ...
    for (const poiCoverage_foreach_range of isochronePruneResults.poiCoverage) {
      const poiFeatureID = poiCoverage_foreach_range.poiFeatureId.substring(0, poiCoverage_foreach_range.poiFeatureId.lastIndexOf("_"));
      const poiFeature = poiMap.get(poiFeatureID);
      if (poiFeature) {
