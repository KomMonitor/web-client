...
export class ReportingService {
  ...
  bakeInCustomInfoToClone() {
    ...
    // update selected template with general settings
    for(let [idx, page] of this.clonedTemplate.pages.entries()) {
      for(let el of page.pageElements) {
        if(el.type.includes("footerCreationInfo-")) {
          el.text = "Erstellt am " + this.generalSettings.creationDate + " von " + this.generalSettings.creator + ", " + this.generalSettings.commune
        }
  ...
  getAreaSpecificPageClone(pageIndex:number):any {
    ...
    let page = structuredClone(this.availableTemplates[this._reportingData$.value.selectedTemplateId].pages[pageIndex]);
  ...
  removeGeoresourcesSection(georesourceId) {
    ...
    let newSectionVal:SectionData = {
      indicators: this._reportingData$.value.sections.indicators,
      georesources: this._reportingData$.value.sections.georesources.filter(e => e.georesourceId!=georesourceId)
    }

    this.setValue({...this._reportingData$.value, sections: newSectionVal});

    // remove from template
    this.selectedTemplate.pages = this.selectedTemplate.pages.filter(e => e.templateSection.georesourceId!=georesourceId);

    if(this.selectedTemplate.pages.length==0)
  ...
  removeIndicatorSection(indicatorId) {
    ...
    // remove from sections
    let newSectionVal:SectionData = {
      indicators: this._reportingData$.value.sections.indicators.filter(e => e.indicatorId!=indicatorId),
    ...
    // remove from template
    this.selectedTemplate.pages = this.selectedTemplate.pages.filter(e => e.templateSection.indicatorId!=indicatorId);
