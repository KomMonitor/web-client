...
export class KommonitorReachabilityCoverageReportsHelperService {
  ...
  private readonly leafletContainer_resolution = this.leafletContainer_width_px / this.leafletContainer_height_px;

  private readonly pdfLeafletImageWidth = 190;
  private readonly pdfLeafletImageHeight = this.pdfLeafletImageWidth / this.leafletContainer_resolution;
  ...
  async generateTotalCoverageReport_focusPoiCoverage(reachabilityScenario: any, indicatorStatistic: any): Promise<void> {
    ...
    this.insertTable_indicator_poi_information(doc, reachabilityScenario, indicatorStatistic, false);
  ...
  async addCoverageInformation_totalCoverage(doc: jsPDF, reachabilityScenario: any, indicatorStatistic: any): Promise<jsPDF> {
    ...
    doc.setFontSize(12);
    const totalCoverageTitle = doc.splitTextToSize("Gesamtergebnis - Versorgung über alle Raumeinheiten", 180);
    ...
    for (const overallCoverageEntry of indicatorStatistic.coverageResult.overallCoverage) {
      ...
      const coverage_absolute = this.kommonitorDataExchangeService.getIndicatorValue_asFormattedText(overallCoverageEntry.coverage[0].absoluteCoverage) + " von " + this.kommonitorDataExchangeService.getIndicatorValue_asFormattedText(indicatorStatistic.coverageResult.timeseries[0].value) + " [" + indicatorStatistic.indicator.unit + "]";
      const coverage_relative = this.kommonitorDataExchangeService.getIndicatorValue_asFormattedText(overallCoverageEntry.coverage[0].relativeCoverage * 100) + " [%]";
    ...
    const leafletMapScreenshot = await this.kommonitorReachabilityMapHelperService.takeScreenshot_image(leafletMapDomId, this.domToImageMoreSettings);
  ...
  insertTable_indicator_poi_information(doc: jsPDF, reachabilityScenario: any, indicatorStatistic: any, insertCoverageTypeShortInformation: boolean): jsPDF {
  ...
  setWeightStrategyTexts(indicatorStatistic: any): void {
    ...
    if (indicatorStatistic.weightStrategy.apiName === "residential_areas") {
      ...
      this.weightStrategyExplanationText += "Pro Raumebene wird nur die Wohnfläche mit den Einzugsgebieten eines Punktes räumlich verschnitten. Die geschätzte Gesamtversorgung einer Raumebene ergibt sich dann aus dem durch die Punkteinzugsgebiete insgesamt überlappenden Anteil an der Wohnfläche innerhalb der Raumebene. Dieses Verfahren berücksichtigt demnach nur die Wohnfläche und liefert daher einen genaueren Schätzwert als der einfache Gesamtflächenanteil. Da keine Einzelpersonen im Verfahren verücksichtigt werden, ist das Ergebnis ausdrücklich als Schätzwert zu interpretieren.";
    } else {
      this.weightStrategyText += "einfacher Gesamtflächenanteil";
      this.weightStrategyExplanationText += "Pro Raumebene wird die Gesamtfläche mit den Einzugsgebieten eines Punktes räumlich verschnitten. Die geschätzte Gesamtversorgung einer Raumebene ergibt sich dann aus dem durch die Punkteinzugsgebiete insgesamt überlappenden Anteil an der Gesamtfläche der Raumebene. Da keine Einzelpersonen im Verfahren verücksichtigt werden, ist das Ergebnis ausdrücklich als Schätzwert zu interpretieren.";
    }
  }
  ...
  insertCoverageType(doc: jsPDF, indicatorStatistic: any): jsPDF {
    ...
    (doc as any).autoTable({
      ...
      body: [
        [this.weightStrategyText, this.weightStrategyExplanationText],
      ],
      theme: 'grid',
      headStyles: this.headStyles,
      bodyStyles: this.bodyStyles,
  ...
  async generateFeatureCoverageReport_focusPoiCoverage(reachabilityScenario: any, indicatorStatistic: any): Promise<void> {
    ...
    const mapParts = this.kommonitorReachabilityMapHelperService.getMapParts_byDomId(leafletMapDomId);
  ...
  sortPoiLayer_byTotalCoverageDesc(poiLayer: any): any[] {
    ...
    poiArray_sorted.sort((a, b) => {
      ...
      const absoluteCoverage_a = isochroneStatistics_a[isochroneStatistics_a.length - 1].overallCoverage[0].absoluteCoverage;

      const isochroneStatistics_b = b.feature.properties.individualIsochronePruneResults;
      const absoluteCoverage_b = isochroneStatistics_b[isochroneStatistics_b.length - 1].overallCoverage[0].absoluteCoverage;
  ...
  async insertSpatialUnitIndividualPage(doc: jsPDF, reachabilityScenario: any, indicatorStatistic: any, spatialUnitLayer: any, domId: string): Promise<jsPDF> {
    ...
    await this.addCoverageInformation_spatialUnitIndividualCoverage(doc, reachabilityScenario, indicatorStatistic, spatialUnitLayer, domId);
  ...
  async addCoverageInformation_spatialUnitIndividualCoverage(doc: jsPDF, reachabilityScenario: any, indicatorStatistic: any, spatialUnitLayer: any, domId: string): Promise<jsPDF> {
    ...
    const leafletMapScreenshot = await this.kommonitorReachabilityMapHelperService.takeScreenshot_image(domId, this.domToImageMoreSettings);
  ...
  async insertPoiIndividualPage(doc: jsPDF, reachabilityScenario: any, indicatorStatistic: any, marker: any, domId: string): Promise<jsPDF> {
    const feature = marker.feature;
    const poiIsochroneLayer = this.kommonitorReachabilityMapHelperService.generateSinglePoiIsochroneLayer(feature);
    this.kommonitorReachabilityMapHelperService.removeSinglePoiIsochroneLayer(domId);
    this.kommonitorReachabilityMapHelperService.addSinglePoiIsochroneLayer(domId, feature, poiIsochroneLayer, true);
    ...
    await this.addCoverageInformation_poiIndividualCoverage(doc, reachabilityScenario, indicatorStatistic, marker, domId);
  ...
  async addCoverageInformation_poiIndividualCoverage(doc: jsPDF, reachabilityScenario: any, indicatorStatistic: any, marker: any, domId: string): Promise<jsPDF> {
    ...
    for (const poiIsochroneStatistic of poiIsochroneStatistics) {
      ...
      const coverage_total_absolute = this.kommonitorDataExchangeService.getIndicatorValue_asFormattedText(poiIsochroneStatistic.overallCoverage[0].absoluteCoverage) + " von " + this.kommonitorDataExchangeService.getIndicatorValue_asFormattedText(indicatorStatistic.coverageResult.timeseries[0].value) + " [" + indicatorStatistic.indicator.unit + "]";
      const coverage_total_relative = this.kommonitorDataExchangeService.getIndicatorValue_asFormattedText(poiIsochroneStatistic.overallCoverage[0].relativeCoverage * 100) + " [%]";
  ...
  async initTotalCoverageLeafletMap(leafletMapDomId: string, reachabilityScenario: any, indicatorStatistic: any): Promise<void> {
    this.kommonitorReachabilityMapHelperService.initReachabilityIndicatorStatisticsGeoMap(leafletMapDomId);

    this.kommonitorReachabilityMapHelperService.replaceIsochroneGeoJSON(leafletMapDomId, this.kommonitorReachabilityHelperService.settings.selectedStartPointLayer.datasetName, this.kommonitorReachabilityHelperService.currentIsochronesGeoJSON, this.kommonitorReachabilityHelperService.settings.transitMode, this.kommonitorReachabilityHelperService.settings.focus, this.kommonitorReachabilityHelperService.settings.rangeArray, this.kommonitorReachabilityHelperService.settings.useMultipleStartPoints, this.kommonitorReachabilityHelperService.settings.dissolveIsochrones);

    const poiDataset = this.kommonitorReachabilityHelperService.settings.selectedStartPointLayer;
    const original_nonDissolved_isochrones = this.kommonitorReachabilityHelperService.original_nonDissolved_isochrones;
    await this.kommonitorReachabilityMapHelperService.replaceReachabilityIndicatorStatisticsOnMap(leafletMapDomId, poiDataset, original_nonDissolved_isochrones, indicatorStatistic);

    this.kommonitorReachabilityMapHelperService.zoomToIndicatorLayer(leafletMapDomId);
  }

  async initPoiIndividualLeafletMap(leafletMapDomId: string, reachabilityScenario: any, indicatorStatistic: any): Promise<void> {
    this.kommonitorReachabilityMapHelperService.initReachabilityIndicatorStatisticsGeoMap(leafletMapDomId);
  ...
  async initSpatialUnitIndividualLeafletMap(leafletMapDomId: string, reachabilityScenario: any, indicatorStatistic: any): Promise<void> {
    this.kommonitorReachabilityMapHelperService.initReachabilityIndicatorStatisticsGeoMap(leafletMapDomId);
  ...
  async generateFeatureCoverageReport_focusSpatialUnitCoverage(reachabilityScenario: any, indicatorStatistic: any): Promise<void> {
    ...
    for (let index = 0; index < indicatorLayer_array.length; index++) {
      ...
      await this.insertSpatialUnitIndividualPage(doc, reachabilityScenario, indicatorStatistic, spatialUnitLayer, leafletMapDomId);
  ...
  aggregatePoisForSpatialUnits(indicatorLayer: any, poiLayer: any): any[] {
    ...
    indicatorArray.sort((a, b) => {
      ...
      const ranges_a = Object.keys(a.feature.properties.overallCoverages).map(item => Number(item));
      const ranges_b = Object.keys(b.feature.properties.overallCoverages).map(item => Number(item));
      const maxRange_a = Math.max(...ranges_a);
      const maxRange_b = Math.max(...ranges_b);
      const coverage_a = a.feature.properties.overallCoverages["" + maxRange_a].absoluteCoverage || 0;
      const coverage_b = b.feature.properties.overallCoverages["" + maxRange_b].absoluteCoverage || 0;
