angular.module('kommonitorReachabilityScenarioHelper', [
  'kommonitorDataExchange',
  'kommonitorReachabilityHelper',
  'kommonitorToastHelper',
]);

angular
  .module('kommonitorReachabilityScenarioHelper')
  .service('kommonitorReachabilityScenarioHelperService', [
    '__env',
    '$rootScope',
    ...
    function (
      ...
      this.setPoiDataset = function (poiDataset) {
        ...
        if (
          kommonitorReachabilityHelperService.settings.isochroneConfig.selectedDate &&
          kommonitorReachabilityHelperService.settings.isochroneConfig.selectedDate.startDate
        ) {
          this.tmpActiveScenario.poiDataset.poiDate =
            kommonitorReachabilityHelperService.settings.isochroneConfig.selectedDate.startDate;
        } else {
      ...
      this.setActiveScenario = function (scenarioDataset) {
        ...
        kommonitorReachabilityHelperService.settings = jQuery.extend(
          true,
          {},
          this.tmpActiveScenario.reachabilitySettings
        );
        kommonitorReachabilityHelperService.currentIsochronesGeoJSON = jQuery.extend(
          true,
          {},
          this.tmpActiveScenario.isochrones_dissolved
        );
        kommonitorReachabilityHelperService.original_nonDissolved_isochrones = jQuery.extend(
          true,
          {},
      ...
      this.configureActiveScenario = function () {
        ...
        this.tmpActiveScenario.isochrones_perPoint =
          kommonitorReachabilityHelperService.original_nonDissolved_isochrones;
      ...
      this.replaceOrAddScenario = function (scenario) {
        ...
        for (let i = 0; i < this.reachabilityScenarios.length; i++) {
          if (this.reachabilityScenarios[i].scenarioName == scenario.scenarioName) {
            ...
            replaced = true;
            kommonitorToastHelperService.displaySuccessToast(
              'Erreichbarkeitsszenario aktualisiert',
              this.tmpActiveScenario.scenarioName
            );
            break;
          }
        }
        ...
        if (!replaced) {
          ...
          kommonitorToastHelperService.displaySuccessToast(
            'Erreichbarkeitsszenario neu angelegt',
            this.tmpActiveScenario.scenarioName
          );
        }
      };
      ...
      this.exportScenarios = function () {
        var scenarios_string = JSON.stringify(this.reachabilityScenarios);
      ...
      this.parseFromFile = function (event) {
        this.reachabilityScenarios = this.reachabilityScenarios.concat(
