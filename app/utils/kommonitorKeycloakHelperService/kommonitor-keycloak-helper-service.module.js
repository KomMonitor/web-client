...
angular
    ...
    .service('kommonitorKeycloakHelperService', ['$rootScope', '$timeout', '$http', '$httpParamSerializerJQLike', 'Auth', '__env',
    function ($rootScope, $timeout, $http, $httpParamSerializerJQLike, Auth, __env) {
        ...
        this.postNewRoles = async function (organizationalUnitName) {
            try {
                ...
                // Add composites
                await this.addCompositeRole_withToken(bearerToken, roleMap[organizationalUnitName + "-editor"], roleMap[organizationalUnitName + "-viewer"]);
                await this.addCompositeRole_withToken(bearerToken, roleMap[organizationalUnitName + "-publisher"], roleMap[organizationalUnitName + "-editor"]);
                await this.addCompositeRole_withToken(bearerToken, roleMap[organizationalUnitName + "-creator"], roleMap[organizationalUnitName + "-publisher"]);
            }
            catch (error) {
                throw error;
            }
...
//# sourceMappingURL=kommonitor-keycloak-helper-service.module.js.map