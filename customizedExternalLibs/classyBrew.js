	...
	var classyBrew = (function () {
		...
		return function () {
			...
			this._classifyEqualInterval = function () {
			    ...
			    for(let i = 0; i <= this.getNumClasses(); i++) {
			        a[i] = val;
			        val += interval;
			    }
			...
			this._classifyStdDeviation = function () {
			    ...
			    if(this.getNumClasses % 2 == 1) {
			    	...
			    	for(let i = supBound + 1; i < this.getNumClasses(); i++) {
			    		var val = a[i-1] + this._stdDev(this.series);
				        a[i] = val;
				    }
			    	...
			    	for(let i = meanBound + 1; i < this.getNumClasses(); i++) {
			    		var val = a[i-1] + this._stdDev(this.series);
				        a[i] = val;
				    }
			    }
			...
			this._classifyJenks = function () {
				...
				for(let i = 0, il = this.numClasses + 1; i < il; i++) {
					kclass.push(0)
				}
			...
			this.getColorInRange = function (num) {
				...
				// [4-5.99, 6-7.99, 8-9]
				var i = 0;
				for(let i; i < this.range.length; i++) {
